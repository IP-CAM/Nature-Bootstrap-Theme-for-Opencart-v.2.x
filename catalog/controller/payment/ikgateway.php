...
class ControllerPaymentIkgateway extends Controller {
    ...
    public function index()
    {
        ...
        $ik_cur = '';
		if($this->currency->getCode() == 'RUB'){
			$ik_payment_amount = number_format($this->currency->format($order_info['total'], $this->config->get('ikgateway_currency'), $this->currency->getValue('RUB'), FALSE), 2, '.', '');
			$ik_cur = 'RUB';
		}else{
        ...
        {
            return $this->load->view('default/template/payment/ikgateway.tpl', $data);
        }
    }
