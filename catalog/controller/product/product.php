...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			$data['text_sertificates_short'] = $this->language->get('text_sertificates_short');
			$data['text_buy_stock'] = $this->language->get('text_buy_stock');
			$data['text_reviews_my'] = $this->language->get('text_reviews_my');
			$data['text_price'] = $this->language->get('text_price');
			
			$data['text_price'] = $this->language->get('text_price');
			$data['all_products'] = $this->language->get('all_products');
			$data['text_skidki'] = $this->language->get('text_skidki');
			...
			$data['product_upc'] = $product_info['upc'];
		     $data['text_russia_only'] = $this->language->get('text_russia_only');
			...
			if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
				...
				$data['price_new'] = $this->currency->format($this->tax->calculate(round($product_info['price'] * 0.77, 2, PHP_ROUND_HALF_UP), $product_info['tax_class_id'], $this->config->get('config_tax')));
			} else {
				$data['price'] = false;
			...
			//получаем скидку чтобы она сука работала в карточе продукта
			//$data['discount']=
			...
			$data['debug'] = $total_data;
			$data['totals'] = array();
			$data['discount'] = 0;
			
			foreach ($total_data as $total) {
				if (isset($total['percent'])){$data['discount'] = $total['percent'];}
				$data['totals'][] = array(
					'title' => $total['title'],
			...
			$data['reviews'] = sprintf("%s" . $this->language->get('text_reviews'), (int)$product_info['reviews']);
			...
			$results = $this->model_catalog_product->getProductRelatedArticle($product_id);
			...
			'short_description' => mb_substr(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8')), 0, 500) . '...',
