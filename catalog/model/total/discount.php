...
class ModelTotalDiscount extends Model {
    ...
    public function getTotal(&$total_data, &$total, &$taxes) {
        if ($this->config->get('discount_status')) {
			...
			$discount_string = $this->config->get('discount_string');
			//$this->log->debug(serialize($discount_string));
            ...
            for ($i = 0; $i < $len; $i++) {
                ...
                if ($discount_from[$i] <= $t && $t < $discount_to[$i]) {
					...
					//$this->log->debug('$discount_val = '.serialize(str_replace('%','',$discount_val)));
					
                    $discounts[] = array(
                        'code' => 'discount',
                        'title' => (!empty($discount_string[$i]))?$discount_string[$i]:str_replace('\.','',sprintf($title_format, $discount_from_currency, $discount_val)),
                        'value' => $value,
                        'sort_order' => $this->config->get('discount_sort_order'),
                        'minus' => true,
                        'percent' => str_replace('%','',$discount_val)
                    );
                }
            }
