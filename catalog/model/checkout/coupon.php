...
class ModelCheckoutCoupon extends Model {
	public function getCoupon($code) {
		...
		if ($coupon_query->num_rows) {
			...
			/*fix добавляем купону  новое поведение
			теперь можно выбрать исключительно незарегестрированых пользователей 
			...
			*/
			...
			if ($coupon_query->row['logged']==1 && !$this->customer->getId()) {
				$status = false;
			}

			if($coupon_query->row['logged']==3 && $this->customer->getId()){
				$status = false;
			}
		...
		if ($status) {
			...
			return array('errorcode'=>$coupon_query->row['logged']);
		}
	}
}