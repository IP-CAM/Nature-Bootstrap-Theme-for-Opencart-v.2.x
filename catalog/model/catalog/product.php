...
class ModelCatalogProduct extends Model {
	...
	public function getProductRelatedArticle($product_id) {
		$product_data = array();
		$query = $this->db->query(
			"SELECT * FROM " . DB_PREFIX . "product_related_article pr LEFT JOIN " . DB_PREFIX .
			"easy_blog_article a ON (pr.related_article_id = a.article_id) LEFT JOIN " . DB_PREFIX .
			"easy_blog_article_description ebad ON (a.article_id = ebad.article_id) WHERE pr.product_id = " . (int)$product_id .
			" AND a.status = 1 AND a.date_modified <= NOW()");

		foreach ($query->rows as $result) {
			$this->load->model('blog/article');
			$product_data[$result['related_article_id']] = $this->model_blog_article->getArticle($result['related_article_id']);
		}
		return $product_data;
	}
