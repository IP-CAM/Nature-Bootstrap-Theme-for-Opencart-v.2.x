...
class ControllerTotalDiscount extends Controller {
    ...
    public function selectModel() {
        if (floatval(VERSION) >= 2.2 && file_exists(DIR_CATALOG . 'model/total/discount22.php')) {
            if (file_exists(DIR_CATALOG . 'model/total/discount.php'))
                unlink(DIR_CATALOG . 'model/total/discount.php');

            if (file_exists(DIR_CATALOG . 'model/total/discount21.php'))
                unlink(DIR_CATALOG . 'model/total/discount21.php');

            rename(DIR_CATALOG . 'model/total/discount22.php', DIR_CATALOG . 'model/total/discount.php');
        }
        else if (floatval(VERSION) < 2.2 && file_exists(DIR_CATALOG . 'model/total/discount21.php')) {
            if (file_exists(DIR_CATALOG . 'model/total/discount.php'))
                unlink(DIR_CATALOG . 'model/total/discount.php');

            if (file_exists(DIR_CATALOG . 'model/total/discount22.php'))
                unlink(DIR_CATALOG . 'model/total/discount22.php');

            rename(DIR_CATALOG . 'model/total/discount21.php', DIR_CATALOG . 'model/total/discount.php');
        }
    }
    ...
    public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->log->debug(serialize($this->request->post));
		...
		if (isset($this->request->post['discount_string']))
            $data['discount_string'] = $this->request->post['discount_string'];
        else
            $data['discount_string'] = $this->config->get('discount_string');
