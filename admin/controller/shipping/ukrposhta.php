...
class ControllerShippingUkrposhta extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('shipping/ukrposhta', 'token=' . $this->session->data['token'], 'SSL')
		);

		$data['action'] = $this->url->link('shipping/ukrposhta', 'token=' . $this->session->data['token'], 'SSL');

		$data['cancel'] = $this->url->link('extension/shipping', 'token=' . $this->session->data['token'], 'SSL');

		if (isset($this->request->post['ukrposhta_total'])) {
			$data['ukrposhta_total'] = $this->request->post['ukrposhta_total'];
		} else {
			$data['ukrposhta_total'] = $this->config->get('ukrposhta_total');
		}
