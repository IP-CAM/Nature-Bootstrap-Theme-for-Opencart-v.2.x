...
class ControllerModuleFeedItems extends Controller {
	...
	public function index() {
		$this->load->language('module/feed_items'); //подключаем наш языковой файл
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) { //если мы нажали "Сохранить"  в панели, мы сохраняем текущие настройки
			$this->model_setting_setting->editSetting('feed_items', $this->request->post);
		...
		$data['action'] = $this->url->link('module/feed_items', 'token=' . $this->session->data['token'], 'SSL');
		$data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');

		//переменная с статусом модуля
        if (isset($this->request->post['feed_items_status'])) {
			$data['feed_items_status'] = $this->request->post['feed_items_status'];
		} else {
			$data['feed_items_status'] = $this->config->get('feed_items_status');
		}
		...
		$this->response->setOutput($this->load->view('module/feed_items.tpl', $data));
	}
